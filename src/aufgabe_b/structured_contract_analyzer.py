...
output_file = 'data/aufgabe_b_results.json'
...
for index, contract in enumerate(contracts):
    ...
    # For this approach, I decided to truncate the text to the first 15,000 characters to prevent crashes.
    raw_text = contract["text"]
    contract_text = raw_text[:15000] if len(raw_text) > 15000 else raw_text
    ...
    try:
        ...
        # if the response is valid, append only the true tags to my final list
        for key, value in response_dict.items():
            if value:
                extracted_tags.append(key)
    ...
    # validation error and general exception handling 
...
average_time_per_contract = total_time / len(contracts)
